...
useEffect(()=>{
    ...
    try {
        ...
        const result = await axios.post("/api/chat/ai-suggestions",{message:lastmessage?.text,role:"customer"})
...
onClick={getSuggestion}
                ><Sparkle size={14}></Sparkle>{loading ? <Loader className='w-5 h-5 animate-spin' />:"AI suggest"}</motion.button>
            </div>
            ...
            <div className='flex-1 overflow-y-auto p-2 space-y-3' ref={chatBoxRef}>
                <AnimatePresence>
                    {messages?.map((msg, index) => (
                        ...
                        >
                            <div className={`px-4 py-2 max-w-[75%] rounded-xl shadow
                                ${
