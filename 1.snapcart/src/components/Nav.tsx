'use client'
import { Boxes, ClipboardCheck, Cross, LogOut, Menu, Package, Plus, PlusCircle, Search, ShoppingCart, ShoppingCartIcon, User, X } from 'lucide-react'
...
<div className='flex items-center gap-3 text-red-300 font-semibold mt-auto hover:bg-red-500/20 p-3 rounded-lg transition-all' onClick={async ()=> signOut({callbackUrl:"/"})}>
    ...
    return (
        <div className='w-[95%] fixed top-4 left-1/2 -translate-x-1/2 bg-gradient-to-r from-green-500 to-green-700 rounded-2xl shadow-lg shadow-black/30 flex justify-between items-center h-20 px-4 md:px-8 z-50'>
            ...
            <Link href={"/"}>
                <span className='text-white font-extrabold text-2xl sm:text-3xl tracking-wide hover:scale-105 transition-transform'>
                    Snapcart
                </span>
            </Link>
            {user.role == "user" && <form className='hidden md:flex items-center bg-white  rounded-full px-4 py-2 w-1/2 max-w-lg shadow-md' onSubmit={handleSearch}>
                <Search className='text-gray-500 w-6 h-5 mr-2' />
                    ...
                    <AnimatePresence>
                        {open &&
                            ...
                            <motion.div
                                ...
                                className='absolute right-0 mt-3 w-56 bg-white rounded-2xl shadow-xl border-gray-200 p-3 z-999'
                            >
