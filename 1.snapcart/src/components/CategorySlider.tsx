...
function CategorySlider() {
    const categories = [
        ...
        {id: 9, name :"Instant & Packed Foods",icon:Box, color: "bg-teal-100"},
  ...
  return (
    ...
    >
      ...
      {showLeft && <button className='absolute left-0 top-1/2 transform -translate-y-1/2 z-10 bg-white shadow-lg hover:bg-green-100 p-2 rounded-full w-10 h-10 flex items-center justify-center transition-all'
      onClick={() => scroll("left")}
...
{showRight && <button className='absolute right-0 top-1/2 transform -translate-y-1/2 z-10 bg-white shadow-lg hover:bg-green-100 p-2 rounded-full w-10 h-10 flex items-center justify-center transition-all' onClick={() => scroll("right")}><ChevronRight className='w-6 h-6 text-green-700'/></button>}
